...
function getLoginReward($uid,$param){	
	...
	if ($rewardtype==4){ //充值后可领取
		...
		$today_in_money=sql_fetch_one_cell("select sum(money) from pay_log where passport='$passport' and passtype='$passtype' and date(from_unixtime(time))='$date'");
		if ($today_in_money<=0)		
...
function getLoginInfo($uid,$param){
	...
	//if ($nobility>=1){ //公士以上才可领取
		...
		$new_pack_number= sql_fetch_one_cell("select gid-50100 from sys_goods where uid = $uid and count=1 and  gid between 50101 and 50110");
