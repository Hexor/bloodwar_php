...
function getCityProduct($uid,$param)
{
	...
	if ($city['goods_iron_add'] > 0)
	{
     ...
     getCityzy($cid,$uid);//此处功能1.23添加：当点击 黄金数量后面的“资源生产”图标 或者 官府 中的“资源生产”图标时能更新资源和产量
...
function setCityProductRate($uid,$param)
{
    ...
    getCityzy($cid,$uid);//此处功能一直存在：当点击修改生产率的时候能更新资源和产量
	if ($food_rate == 100)
	{
...
{
	//$cid = array_shift($param);
	$city = sql_fetch_one("select * from sys_city_res_add where cid=".$cid);//重要不要修改，影响生产
	if (empty($city))
	{
		sql_query("insert into sys_city_res_add (cid) values ('$cid')");
		$city = sql_fetch_one("select * from sys_city_res_add where cid='$cid'");
	...
	$iron_add_base = GLOBAL_IRON_RATE * $iron_all_people * GAME_SPEED_RATE;
	//资源上限  1.24添加
	$food_max = GLOBAL_FOOD_MAX_RATE * $food_all_people * GAME_SPEED_RATE;
	$wood_max = GLOBAL_WOOD_MAX_RATE * $wood_all_people * GAME_SPEED_RATE;
	$rock_max = GLOBAL_ROCK_MAX_RATE * $rock_all_people * GAME_SPEED_RATE;
	$iron_max = GLOBAL_IRON_MAX_RATE * $iron_all_people * GAME_SPEED_RATE;
	...
	if ($chiefHero['chiefhid'] > 0)    //有将领的情况下
	{
		//“将领加成率”=内政加成+基本内政+内政始终加成
		//1+40+0=41
		$chief_add = $chiefHero['affairs_add']+$chiefHero['affairs_base']+$chiefHero['affairs_add_on'];
		//将领统帅能力=等级统率（即1级1统率）+基本统帅（可能是科技产生的统率）+统帅加成（可能是装备产生的统率）
		//2+0+0=2
		$heroCommand = $chiefHero["level"]+$chiefHero["command_base"]+$chiefHero["command_add_on"];
		//城市人口上限
		//15500
		$cityPeopleMax = sql_fetch_one_cell("select people_max from mem_city_resource where cid=".$cid);
		//虎符加成：默认为1；如果使用虎符，加成为1.5
		//1
		$hufu=1;
		if(sql_check("select hid from mem_hero_buffer where hid='$chiefHero[chiefhid]' and buftype=1 and endtime>unix_timestamp()"))
		{
		...
		//人口加成率=将领统帅能力*10*（100*虎符加成+统帅科技*10）/（人口上限+1）
		//2*10*（100*1+0）/(15500+1)=0.129....
		$peoplerate = $heroCommand*10.0* (100*$hufu + $leaderTechLevel * 10) / ($cityPeopleMax+1);
        //如果人口加成率大于1，则加成率取值1。一般情况将领加成率=“将领加成率”*人口加成率
		// 41*0.129=5.28998
		if ($peoplerate > 1.0) $peoplerate = 1.0;
		$chief_add =  $chief_add * $peoplerate;

		//文曲星符增加内政25%
		//5.28998*1.25=6.61247
		if(sql_check("select hid from mem_hero_buffer where hid='$chiefHero[chiefhid]' and buftype=2 and endtime>unix_timestamp()"))
		{
			$chief_add=$chief_add*1.25;//一般获得的将领加成率计算资源加成要除以100再乘基础产量
		}
	}
...
/*
	...
	//宝物加成,目前宝物时间到了加成状态仍在，需进一步调试，调试好请删除此行
	$goods_food_add = sql_fetch_one_cell("select goods_food_add from sys_city_res_add where cid=".$cid);
	$goods_wood_add = sql_fetch_one_cell("select goods_wood_add from sys_city_res_add where cid=".$cid);
	$goods_rock_add = sql_fetch_one_cell("select goods_rock_add from sys_city_res_add where cid=".$cid);
	$goods_iron_add = sql_fetch_one_cell("select goods_iron_add from sys_city_res_add where cid=".$cid);
     ...
     //城市可用人口
	 $tm = sql_fetch_one_cell("select lastupdate from mem_city_resource where cid='$cid'");
     //上次刷新时间
	 ...
	 //下面是产量计算公式
	 //$foodadd=$food_add_base*($city['food_rate']/100)*$bl+$food_add_base*$food_add_rate_technic/100+100;
     //$woodadd=$wood_add_base*($city['wood_rate']/100)*$bl+$wood_add_base*$wood_add_rate_technic/100+100;
     //$rockadd=$rock_add_base*($city['rock_rate']/100)*$bl+$rock_add_base*$rock_add_rate_technic/100+100;
     //$ironadd=$iron_add_base*($city['iron_rate']/100)*$bl+$iron_add_base*$iron_add_rate_technic/100+100;
      //上面4种资源的产量还有 宝物加成、野地加成、将领加成 没添加，军队加成暂设为0
	  /* 资源产量=1基础产量+2自然加成+3科技加成-4军队加成+5将领加成+6野地加成+7宝物加成 
	     1基础产量=生产能力*开工率（正常）
	     2自然加成=100(正常)
		 3科技加成=基础产量*科技加成率（正常）
		 4军队加成=军队数量*单兵消耗（正常，士兵吃粮已经在StatFunc.php的第84行中计算进去了）
		 5将领加成=基础产量*将领加成率（正常）
		 6野地加成=基础产量*野地加成率（未完成，野地加成率无野地暂时无法计算）
		 7宝物加成=基础产量*宝物加成率（正常，宝物加成率好像是固定的0.25）
	  */
		// 定义各资源代号
		//粮食
		$f1=$food_add_base*($city['food_rate']/100)*$bl;
		$f2=100;
		$f3=$food_add_base*($city['food_rate']/100)*$bl*$food_add_rate_technic/100;
		$f4=$food_army_use;//士兵吃粮$f4已经在StatFunc.php的第84行中计算进去了
		$f5=$food_add_base*($city['food_rate']/100)*$bl*$chief_add/100;
		$f6=$food_add_base*($city['food_rate']/100)*$bl*$field_food_add/100;//野地加成率无野地暂时无法计算
		$f7=$food_add_base*($city['food_rate']/100)*$bl*$goods_food_add/100;
		
		//木头
		$w1=$wood_add_base*($city['wood_rate']/100)*$bl;
		$w2=100;
		$w3=$wood_add_base*($city['wood_rate']/100)*$bl*$wood_add_rate_technic/100;
		$w5=$wood_add_base*($city['wood_rate']/100)*$bl*$chief_add/100;
		$w6=$wood_add_base*($city['wood_rate']/100)*$bl*$field_wood_add/100;//野地加成率无野地暂时无法计算
		$w7=$wood_add_base*($city['wood_rate']/100)*$bl*$goods_wood_add/100;
		
		//石头
		$r1=$rock_add_base*($city['rock_rate']/100)*$bl;
		$r2=100;
		$r3=$rock_add_base*($city['rock_rate']/100)*$bl*$rock_add_rate_technic/100;
		$r5=$rock_add_base*($city['rock_rate']/100)*$bl*$chief_add/100;
		$r6=$rock_add_base*($city['rock_rate']/100)*$bl*$field_rock_add/100;//野地加成率无野地暂时无法计算
		$r7=$rock_add_base*($city['rock_rate']/100)*$bl*$goods_rock_add/100;
		
		//铁锭
		$i1=$iron_add_base*($city['iron_rate']/100)*$bl;
		$i2=100;
		$i3=$iron_add_base*($city['iron_rate']/100)*$bl*$iron_add_rate_technic/100;
		$i5=$iron_add_base*($city['iron_rate']/100)*$bl*$chief_add/100;
		$i6=$iron_add_base*($city['iron_rate']/100)*$bl*$field_iron_add/100;//野地加成率无野地暂时无法计算
		$i7=$iron_add_base*($city['iron_rate']/100)*$bl*$goods_iron_add/100;
		  
	  
	 $foodadd=$f1+$f2+$f3+$f5+$f6+$f7;//士兵吃粮$f4已经在StatFunc.php的第84行中计算进去了
     $woodadd=$w1+$w2+$w3+$w5+$w6+$w7;
     $rockadd=$r1+$r2+$r3+$r5+$r6+$r7;
     $ironadd=$i1+$i2+$i3+$i5+$i6+$i7;
     ...
     $ironad=$ironadd/3600;
	  ...
	  sql_query("update mem_city_resource set `changing`=1,people_working='$aa',food=food+'$foodad'*(unix_timestamp()-$tm),wood=wood+'$woodad'*(unix_timestamp()-$tm),rock=rock+'$rockad'*(unix_timestamp()-$tm),iron=iron+'$ironad'*(unix_timestamp()-$tm),food_add='$foodadd',wood_add='$woodadd',rock_add='$rockadd',iron_add='$ironadd',food_max='$food_max',wood_max='$wood_max',rock_max='$rock_max',iron_max='$iron_max',food_army_use='$f4',lastupdate=unix_timestamp() where cid='$cid'");
