...
function getDefenceSpeedRate($cid)
{
    ...
    {
        $speed_add += 10 * $buildingTechLevel;
    }
...
function doGetReinforceQueue($uid,$cid)
{
	...
	foreach($queues as $queue)
	{
		...
		if ($state->state == 1)
		{
			...
			if ($state->time_left < 1)
			{
				...
				//在此插入更新城防方法update_defence($cid);
				update_wall($cid,$queue['count'],$queue['did'],$queue['id']);
				return doGetReinforceQueue($uid,$cid);//调出新队列
				//$state->time_left = 1;
				//$state->state = 0;
				//continue();//如果已经城防完成就结束
			}
		}
		//
	...
	//file_put_contents('c:/state_rt_'.time(), var_export($ret,true));
	return $ret;
	//return null;
}
//传入城市id 城防数量 城防类型 城防队列 建筑标示
 function update_wall($cid,$count,$did,$qid)
 {
	//查找是否已经存在城防信息
	//file_put_contents('c:/update', 'data');
	if(!sql_check("select * from sys_city_defence where `cid`='$cid' and `did`='$did'"))
	{
		//不存在 则插入
			sql_query("insert into sys_city_defence (`cid`,`did`,`count`) values ('$cid','$did','$count')");

	}else{
		//file_put_contents('c:/xxxxxxxxxxxxxxx', 'xxx');
		//存在则更新
		sql_query("update sys_city_defence set `count`=`count`+'$count' where `cid`='$cid' and `did`='$did'"); 
	}
...
//如果还有队列在排队，则开始新的建造
	if (sql_check("select * from sys_city_reinforcequeue where `cid`='$cid'"))	//如果队列已经在造了
	{
...
function startReinforceQueue($uid,$cid,$param)
{
	...
	{                                     
		...
		{                            
			$pre_building_id = $condition['pre_id'];
			if ($pre_building_id == ID_BUILDING_WALL)
            {
			   $curr_building_level = sql_fetch_one_cell("select `level` from sys_building where cid='$cid' and `bid`='$pre_building_id'");
			}
            else
            {
			    $curr_building_level = sql_fetch_one_cell("select max(`level`) from sys_building where cid='$cid' and `bid`='$pre_building_id'");
...
function accReinforceQueue($uid,$cid,$param)
{
	...
	if(!checkGoods($uid,64)) throw new Exception($GLOBALS['accDefence']['no_goods']);
	...
	if ($queue['state'] == 1)	//正在建设中
	{
		sql_query("update sys_city_reinforcequeue set needtime=(unix_timestamp()-state_starttime)+ceil((needtime+state_starttime-unix_timestamp())*0.7), accmark=1 where id='$queue[id]'");
		sql_query("update mem_city_reinforce set state_endtime=unix_timestamp()+floor((state_endtime-unix_timestamp())*0.7) where id='$queue[id]'");
	}
	else
	{
		sql_query("update sys_city_reinforcequeue set needtime=ceil(needtime*0.7), accmark=1 where id='$queue[id]'");
	}
