...
function createRole($uid,$param)
{
	...
	sql_insert("insert into sys_mail_sys_box (`uid`,`contentid`,`title`,`read`,`posttime`) values ('$uid','12','客服帮助','0',unix_timestamp())");
	sql_insert("insert into sys_mail_sys_box (`uid`,`contentid`,`title`,`read`,`posttime`) values ('$uid','13','欢迎进驻《热血三国》','0',unix_timestamp())");
	sql_insert("insert into sys_mail_sys_box (`uid`,`contentid`,`title`,`read`,`posttime`) values ('$uid','14','热血三国游戏说明','0',unix_timestamp())");
	sql_insert("insert into sys_mail_sys_box (`uid`,`contentid`,`title`,`read`,`posttime`) values ('$uid','5','新手保护提醒','0',unix_timestamp())");
	...
	*/
	...
	sql_query("insert into sys_goods (uid,gid,count) values ('$uid','50101',1)");
...
function changeUserState($uid,$param)
{
	...
	/*if(defined("USER_FOR_51")==false || !USER_FOR_51 ){
	...
	if($targetstate==6)
	{
		...
		foreach($mycities as $city)
		{
			...
			$comma=",";
			if(intval($city['type'])==0){
				$mycommoncities .=$commoncomma.$city['cid'];
				$commoncomma=",";
			}
		}
		//建筑在升级，不能休假
		...
		if($mycommoncities!="")
		sql_query("update mem_city_resource set vacation=1 where cid in ($mycommoncities)");
