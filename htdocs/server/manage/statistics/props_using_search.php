<?php
	...
	$shopinfo = sql_fetch_one("select id,price from cfg_shop where name = '$props'");
	...
	if(!empty($shopinfo)){
		...
		from log_goods where `type`=0  and  gid='$shopinfo[id]' and time >= unix_timestamp($startday) and 
