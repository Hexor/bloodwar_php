<?php
    ...
    foreach($union_users as $user){
	    ...
	    sql_query("insert into log_armor (uid,aid,count,time,type) values ('$user[uid]','$request[aid]','$request[count]',unix_timestamp(),5) ON DUPLICATE KEY UPDATE `count`=`count`+'$request[count]'");
    }
