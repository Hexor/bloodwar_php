...
function addBattleHero($uid, $hid, $cnt, $cityId) {
	...
	$val = sprintf ( "'%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s'", 
	$uid, $hero ['name'], $hero ['npcid'], $hero ['sex'], $hero ['face'], $cityId, 0, $hero ['level'], 
	$total_exp, $affairs, $bravery, $wisdom, $affairs_add, $bravery_add, $wisdom_add, $hero ['loyalty'], $command );
	
	$sql = "insert into sys_city_hero (`uid`,`name`,`npcid`,`sex`,
	`face`,`cid`,`state`,`level`,`exp`,`affairs_base`,`bravery_base`,`wisdom_base`,`affairs_add`,`bravery_add`,
	`wisdom_add`,`loyalty`, `command_base`) values ($val)";
	...
	sql_query ( "insert into log_goods (uid,gid,count,time,type) values ('$uid','$hid','$cnt',unix_timestamp(), 10)" ); //属于商场购买,但是属于勋章换来的Hero
	"select * from log_goods, ";
}
