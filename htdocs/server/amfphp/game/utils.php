...
function getCityNamePosition($cid)
{
	...
	if(empty($cityname)){
		$wid = cid2wid($cid);
		$cityname = sql_fetch_one_cell("select c.name from mem_world m left join cfg_world_type c on c.type=m.type where m.wid='$wid'");
	}
...
function upbuilding($cid)
{
	...
	//sql_query("delete sys_building where `state` =0 and `level`=0");//删除0级建筑 by近水
