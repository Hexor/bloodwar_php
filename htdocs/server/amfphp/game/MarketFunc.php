...
function buyFromMerchant($uid,$cid,$param)
{
   ...
   // $usermoney = sql_fetch_one_cell("select money from sys_user where uid='$uid'");
	...
	if ($paytype==0&&($userMoney < 5))	throw new Exception($GLOBALS['buyFromMerchant']['no_enough_YuanBao']);
	if ($paytype==1&&($userGift < 5))	throw new Exception($GLOBALS['buyFromMerchant']['no_enough_Gift']);
...
function sellToMerchant($uid,$cid,$param)
{
	...
	//礼金
	$userGift=$userInfo['gift'];
	if ($paytype==0&&($userMoney < 5))	throw new Exception($GLOBALS['sellToMerchant']['no_enough_YuanBao']);
	if ($paytype==1&&($userGift < 5))	throw new Exception($GLOBALS['sellToMerchant']['no_enough_Gift']);
...
function addAutoTrans($uid,$cid,$param){
    ...
    $fromcid = array_shift($param);
    $tocid = array_shift($param);
    $resType = array_shift($param);
    $count = array_shift($param);
    $transType = array_shift($param);
