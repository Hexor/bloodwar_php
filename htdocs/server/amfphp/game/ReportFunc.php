...
function getUnreadReport($uid,$param)
{
    ...
    if($reportCount>0)
    {
    	...
    	$ret[] = sql_fetch_rows("select id,origincid,origincity,happencid,happencity,title,type,time,`read`,battleid from sys_report where uid='$uid' and `read`=0 and state=0 order by `id` desc limit $pagestart,".REPORT_PAGE_CPP);
    }
    else 
    {
    	$ret[]=array();
...
function getReport($uid,$param)
{
    ...
    {
    	$pagestart = $page * REPORT_PAGE_CPP;
    	$ret[] = sql_fetch_rows("select id,origincid,origincity,happencid,happencity,title,type,time,`read`,battleid from sys_report where uid='$uid' and type='$type' and state=0 order by `id` desc limit $pagestart,".REPORT_PAGE_CPP);
    }
    else 
    {
    	$ret[]=array();
