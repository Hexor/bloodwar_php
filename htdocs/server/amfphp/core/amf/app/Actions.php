...
function adapterAction (&$amfbody) {
	...
	if (strpos($target, "http://") === false && strpos($target, "https://") === false) { // check for a http link which means web service
		...
		if ($lpos === false) {
			...
			//file_put_contents('c:/y'.rand(1,1000).time(),var_export($body,true));//抓取传入的动作
			...
			//file_put_contents('c:/z'.time(),$methodname.'!!!!!'.$trunced.'!!!!!'.$lpos);
			if ($lpos === false) {
				$classname = $trunced;
	...
	//file_put_contents('c:/m'.time(),$methodname);
	return true;
} 
