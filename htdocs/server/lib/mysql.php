...
// FOR A PARTICULAR PURPOSE.
	...
	function remote_sql_connect()
	{
		if (!isset($GLOBALS['remotedb']) || $GLOBALS['remotedb'] == NULL)
		{
			$GLOBALS['remotedb'] = new database(database::makeDSN(remotedb_RDBMS, remotedb_Server, remotedb_Port, remotedb_Username, remotedb_Password, remotedb_Database));
			$dbcharset = $GLOBALS['remotedbcharset'];
			$GLOBALS['remotedb']->query("SET character_set_connection=$dbcharset, character_set_results=$dbcharset, character_set_client=binary");
		}		
	}
	function remote_sql_query( $inQueryString, $Db = -1 )
	{
		remote_sql_connect();
		global $remotedb;
		$return = $remotedb->query($inQueryString);
		return $return;
	}
	function remote_sql_fetch_one($inQueryString)
	{
		remote_sql_connect();
		global $remotedb;
		$return = $remotedb->fetch_one($inQueryString);
		return $return;
	}
	function remote_sql_fetch_one_cell($inQueryString, $inField = 0)
	{
		remote_sql_connect();
		global $remotedb;
		$return = $remotedb->fetch_one_cell($inQueryString, $inField);
		return $return;
	}
	function sql_use_database($dsnname)
	{
		unset($GLOBALS['defaultdb']);
		$GLOBALS['currentdb'] = $dsnname;
	}
	function sql_connect()
	{
		if (!isset($GLOBALS['defaultdb']) || $GLOBALS['defaultdb'] == NULL)
		{
			...
			$GLOBALS['defaultdb']->query("SET character_set_connection=$dbcharset, character_set_results=$dbcharset, character_set_client=binary");
		}
	}
