...
function loadUserArmor($uid,$param)
{
	$ret = array();
	$ret[]=50;
    $ret[] = sql_fetch_rows("select * from sys_user_armor a left join cfg_armor c on c.id=a.armorid where a.uid='$uid' and a.`hid`=0");
    return $ret;
}
...
function loadUserPartArmor($uid,$param)
{
	...
	$ret=array();
	$ret[]=sql_fetch_rows("select * from sys_user_armor a , cfg_armor c where a.uid='$uid' and a.`hid`=0 and c.id=a.armorid and c.part=$part");
	return $ret;
}
...
function doGetHeroArmor($hid)
{
	$ret=array();
	$ret[]=$hid;
	$ret[]=sql_fetch_rows("select * from sys_hero_armor h left join sys_user_armor u on u.sid=h.sid and u.hid=h.hid left join cfg_armor c on c.id=u.armorid where h.hid='$hid'");
	return $ret;
}
...
function repairAllArmor($uid,$param)
{
	...
	{
		regenerateHeroAttri($uid,$armors[0]['hid']!=0);
	}
	$ret=array();
	$ret[]=$type;
	$ret[]=$cid;
